...
class FilterByEndDate extends Component {
  state = {
    tournaments: [],
    showResults: null,
    time: null
  ...
  getSeriesdate = date => {
    ...
    let getSpecificserie = this.state.tournaments.filter(
      item => item.date_end === date
    );
    console.log(getSpecificserie + " date_end");
...
export default FilterByEndDate;
